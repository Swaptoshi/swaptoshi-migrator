...
export const governanceGenesisStoreSchema = {
	...
	properties: {
		boostedAccountSubstore: {
			fieldNumber: 1,
			type: 'array',
			items: genesisSchemaBuilder(boostedAccountStoreSchema, [
		...
		castedVoteSubstore: {
			fieldNumber: 2,
			type: 'array',
			items: genesisSchemaBuilder(castedVoteStoreSchema, [
				{ key: 'address', dataType: 'bytes', format: 'klayr32' },
			]),
		},
		delegatedVoteSubstore: {
			fieldNumber: 3,
			type: 'array',
			items: genesisSchemaBuilder(delegatedVoteStoreSchema, [
				{ key: 'address', dataType: 'bytes', format: 'klayr32' },
			]),
		},
		nextAvailableProposalIdSubstore: {
		...
		proposalVoterSubstore: {
			fieldNumber: 5,
			type: 'array',
			items: genesisSchemaBuilder(proposalVoterStoreSchema, [
				{ key: 'proposalId', dataType: 'uint32' },
			]),
		},
		proposalSubstore: {
			fieldNumber: 6,
		...
		queueSubstore: {
			fieldNumber: 7,
			type: 'array',
			items: genesisSchemaBuilder(proposalQueueStoreSchema, [
		...
		voteScoreSubstore: {
			fieldNumber: 8,
			type: 'array',
			items: genesisSchemaBuilder(voteScoreStoreSchema, [
				{ key: 'address', dataType: 'bytes', format: 'klayr32' },
			]),
		},
		configSubstore: {
